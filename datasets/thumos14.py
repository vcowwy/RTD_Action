...
class ThumosDetection(paddle.io.Dataset):
    ...
    def get_data(self, video: VideoRecord):
        ...
        og_locations = torch2paddle.create_tensor([location for location in video.locations])

        vid_feature = paddle.load(os.path.join(self.feature_folder, vid))
        ft_idxes = [min(i // 8, vid_feature.shape[0] - 1)
        ...
        if video.absolute_position:
            locations = torch2paddle.create_tensor(
        ...
        targets = {
            'labels': [],
            'boxes': [],
            'video_id': torch2paddle.create_tensor([self.video_dict[vid]])}
...
def collate_fn(batch):
    ...
    snippet_fts = paddle.zeros([batch_size, max_props_num, ft_dim]).requires_grad_(False)
    locations = paddle.zeros([batch_size, max_props_num, 1], dtype=paddle.float64).requires_grad_(False)
    s_e_scores = paddle.zeros([batch_size, max_props_num, 2]).requires_grad_(False)
