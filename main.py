...
from util.t2p import DistributedSampler, DataLoader
...
def get_args_parser():
    ...
    parser.add_argument('--feature_path', default='./data/I3D_features', type=str)
...
def main(args):
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if float(test_stats['50']) > best_ar50:
            ...
            checkpoint_path = output_dir / 'checkpoint_best_ar50.pdparams'
            utils.save_on_master({'model': model_without_ddp.state_dict(),
                                  'optimizer': optimizer.state_dict(),
                                  'lr_scheduler': lr_scheduler.state_dict(),
                                  'epoch': epoch,
        ...
        if current_sum_ar > best_sum_ar:
            ...
            with (output_dir / 'log_best_sum_ar.txt').open('w') as f:
                f.write(json.dumps(log_stats) + '\n')
            checkpoint_path = output_dir / 'checkpoint_best_sum_ar.pdparams'
            utils.save_on_master({'model': model_without_ddp.state_dict(),
                                  'optimizer': optimizer.state_dict(),
                                  'lr_scheduler': lr_scheduler.state_dict(),
                                  'epoch': epoch,
